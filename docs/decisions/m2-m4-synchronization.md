...
| Retry              | API role already manages `campusmeet-*` one-shot schedules; Scheduler role currently invokes Reminder only | Reuse naming/payload convention; add scoped Google-worker invoke role/permission |
| API                | Existing authenticated proxy and success/error envelope                                                    | Add manual retry route and read-side summary                                     |
| Google             | `GoogleCalendarAdapter` is a throwing placeholder                                                          | Implement OAuth-aware reconcile adapter and failure mapping                      |
