...
| `infra/data-foundation.spec.json` | Manifest dễ review của 17 bảng DynamoDB |
...
- `infra/data-foundation.spec.json` giữ tên bảng, key, GSI và TTL ở dạng manifest dễ review.
- `scripts/prepare-data-foundation.mjs` sinh `.aws-sam/data-foundation.generated.json` với `DeletionPolicy: Retain`, `UpdateReplacePolicy: Retain`, SSE, PITR/deletion parameters và dependency tuần tự để dùng khi validate/import/deploy.
