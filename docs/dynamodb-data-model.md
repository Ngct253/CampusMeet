# Mô hình dữ liệu DynamoDB CampusMeet v2
...
Các tên `meeting-data` và `task-data` được chọn để không đụng hai bảng legacy `campusmeet-dev-meetings` và `campusmeet-dev-tasks` đã được tạo trước đó.
...
## 12. Chuyển từ 17 bảng legacy

17 bảng cũ không được xóa ngay.

1. Kiểm tra đúng account `604360241374` và Region `ap-southeast-1`.
2. Chạy audit read-only để lấy `ItemCount`, table status, backup/PITR và ARN.
3. Nếu bảng có dữ liệu, tạo on-demand backup hoặc export S3 trước khi thay đổi.
4. Deploy stack 5 bảng mới bằng tên không đụng legacy.
5. Chạy verify schema 5 bảng.
6. Cập nhật backend environment và repository adapter sang key contract v2.
7. Chạy smoke test core + M5.
8. Chỉ sau khi xác nhận không có code đọc/ghi legacy mới xóa 17 bảng cũ.

Không import 17 bảng cũ vào stack v2 vì schema vật lý khác. Nếu có dữ liệu thật, migration phải chuyển đổi item theo mapping nghiệp vụ, không copy nguyên record một cách mù quáng.

## 13. Điều kiện hoàn thành data layer
...
- cleanup legacy đã có backup/evidence và được review.
