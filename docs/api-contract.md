...
- Các producer nội bộ cần ghi nội dung đã duyệt dùng capability M4 `ImmutableUserContentWriter`: object key suy ra ổn định từ namespace/group/resource/version, backend ghi bằng `If-None-Match: *` kèm SHA-256. Replay phải đọc và băm lại object hiện có; cùng bytes/content-type/metadata trả success, khác biệt trả `USER_CONTENT_INTEGRITY_ERROR`. Capability này không chứa logic Transcript.
- `PersistedAIJobStarter` khởi động chính AIJob đã lưu bằng execution name suy ra từ `aiJobId`. Nó chữa `ORCHESTRATION_START_FAILED`, coi repeated recovery là idempotent, và dùng `DescribeExecution` để đối chiếu state-machine ARN cùng input khi kết quả `StartExecution` không rõ ràng; không tạo logical AIJob thứ hai.
...
Conversion không yêu cầu `Idempotency-Key`. Task ID được sinh deterministic từ `meetingId + actionItemId`, không gồm actor. Backend dùng một `TransactWriteItems` để conditional Put Task vào `task-data` và immutable Minutes version N+1 có `actionItem.taskId` vào `meeting-data`. Nếu latest Action Item đã có `taskId`, backend consistent-read và xác minh Task có đúng `sourceMeetingId`/`sourceActionItemId`, rồi trả lại Task cùng latest Minutes mà không tạo version mới; kiểm tra replay này xảy ra trước stale `expectedMinutesVersion`. Task tạo ra độc lập sau conversion: sửa/xóa Action Item không sửa/xóa Task.
