...
describe('validated API requests', () => {
  it('parses a valid body and applies schema defaults', () => {
    const result: MeetingChatRequest = parseBody(
      event(JSON.stringify({ question: 'Tóm tắt giúp tôi' })),
  ...
  it('returns a safe 400 error for malformed input', () => {
    expect(() => parseBody(event('{'), meetingChatRequestSchema)).toThrow(
      'Body phải là JSON hợp lệ.',
    );
    expect(() =>
      parseBody(event(JSON.stringify({ question: '' })), meetingChatRequestSchema),
    ).toThrow('Dữ liệu yêu cầu không hợp lệ.');
  });

  it('requires a bounded idempotency key', () => {
    expect(requireIdempotencyKey(event(undefined, { 'idempotency-key': 'request-1' }))).toBe(
      'request-1',
    );
